...
This project investigates five growth leakage areas — acquisition, conversion, retention, product mix, and fulfillment — using six months to build a complete analytical picture from raw data to prioritized experiments.
...
**The third finding requires instrumentation before action.** The 15% cancellation rate represents ~$1.63M in lost gross revenue, but the root cause is unknown. No category, order value band, customer type, or distribution center shows elevated cancellation rates. The right move is to capture cancellation reasons before designing any intervention.
...
| Unit economics waterfall | Complete | 1 chart | $10.86M gross → $4.23M estimated net margin (39% of gross) |
...
Of $10.86M in gross revenue over the dataset period, only $4.23M reaches estimated net margin after post-purchase losses and product cost. Cancellations and returns alone consume nearly $2.7M (25% of gross), and the combined loss rate has been structurally stable over time — not a trend that is improving or worsening.
...
Of 681,667 total sessions, 432,099 (63%) added to cart. Of those, only 181,667 (42%) completed a purchase — 250,432 cart sessions ended without a purchase.
...
Only 37.7% of customers ever make a second purchase. The other 62.3% — roughly 50,000 buyers — purchase once and disappear.
...
Several growth hypotheses were tested and did not hold up. These negative results are part of the analytical value — they narrow the action space significantly.
...
Outerwear & Coats, Jeans, and Sweaters are the three largest revenue drivers. Jeans (#2 in revenue) carries a 5+ percentage point margin penalty relative to the platform average — the single largest margin leak in the catalog.

![Top Categories Revenue](outputs/figures/top_categories_by_gross_revenue.png)

Blazers & Jackets (62%), Accessories (60%), and Skirts (60%) are the most margin-efficient categories — all three are significantly underscaled relative to their margins.
...
Email is the largest traffic source by session volume. All five channels show a nearly identical funnel shape — the pattern at right is the same regardless of source.
...
Four experiment designs are grounded in the scorecard above. Each includes: hypothesis, target segment, control/treatment definition, primary metric, guardrail metrics, upside estimate, and risk caveats.
...
**EXP-003 and EXP-002 can run in parallel** — both use email infrastructure and measure independent outcomes.
